...
import worldwidePhoto from './홈페이지용사진/CHILE_SITE.jpg';
import './App.css';
...
function App() {
  ...
  return <main>
    <header className="nav-wrap"><Brand/><button className="menu-button" onClick={() => setMenuOpen(!menuOpen)} aria-label="메뉴 열기"><span></span><span></span></button><nav className={menuOpen ? 'open' : ''}><button onClick={() => scrollTo('about')}>회사소개</button><button onClick={() => scrollTo('service')}>전문 서비스</button><button onClick={() => scrollTo('heritage')}>프로젝트 경험</button><button onClick={() => scrollTo('network')}>글로벌 네트워크</button></nav><a className="quote" href="mailto:roks@roks.kr">운송 문의 <Icon name="arrow"/></a></header>
